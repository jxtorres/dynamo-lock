...
test('first test in suite mocking AWS Communication', async () => {
    ...
    AWSMock.mock('DynamoDB', 'putItem', (params, callback) => {
      console.log('DynamoDB', 'putItem', 'mock called, caller object is under test');
    ...
    assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("TableName", 'dynamo_lock_' + "orion")), 'should have an arg with Expected Table Name Property');
    assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("Item", sinon.match.has("AlbumTitle", sinon.match("S", ".*")))), 'should have an item with some albumtitle structure, regexp');
