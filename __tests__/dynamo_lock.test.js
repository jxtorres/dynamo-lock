...
test('obtainLock Writes an item with the correct lock name', async () => {
  ...
  assert.isTrue(updateSpy.calledOnce, 'test subject should have invoked the spy once, trying to modify DynamoDB Table');
  assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("TableName", 'dynamo_locks')), 'should have an arg with Expected Table Name Property');
  assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("Item", sinon.match.has("LockName", sinon.match.has("S", "dynamo_lock_orion")))), 'should have an item with some albumtitle structure, regexp');
})
...
test('obtainLock only calls for TTL Settings on first invocation ', async () => {
  ...
  assert.isTrue(updateSpy.calledOnce, 'test subject should have invoked the spy once, trying to fetch settings');
  assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("TableName", 'dynamo_locks')), 'should have an arg with Expected Table Name Property');
})
...
test('refreshLock Writes an expiration Time That is 1 hour plus NOW by default', async () => {
  ...
  assert.isTrue(updateSpy2.calledTwice, 'test subject should have invoked the spy twice, trying to modify DynamoDB Table');
...
test('releaseLock Writes to the Dynamo Lock Table', async () => {
  ...
  assert.isTrue(updateSpy.calledTwice, 'test subject should have invoked the spy twice, trying to modify DynamoDB Table');
  assert.isTrue(updateSpy.calledWithMatch(sinon.match.has("TableName", 'dynamo_locks')), 'should have an arg with Expected Table Name Property');
})
